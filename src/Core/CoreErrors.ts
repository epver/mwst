...
export class CoreException extends Error {
  constructor(
    private readonly response: { status: number, message: string, result?: any }
  ) {
    super();
  ...
  public static create(body?: string | any, status = 0, message = '') {
    if (typeof body === 'object' && !Array.isArray(body)) {
      return {status, message, result: body};
...
export class LocalParserError extends CoreException{
  constructor(body?: string | any, status = 0) {
    super(CoreException.create(body, status, '[0] local parser error.'));
  }
}
